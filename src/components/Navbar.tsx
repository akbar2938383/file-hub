        ...
        <div className="flex items-center gap-1 sm:gap-1.5 shrink min-w-0 overflow-x-auto no-scrollbar py-0.5 max-w-full">
          ...
          {onToggleTheme && (
            <button
              ...
              className="p-1 sm:p-2 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg sm:rounded-xl transition-colors shrink-0"
            >
              {isDark ? (
                <Sun className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-500 hover:text-amber-400" />
              ) : (
                <Moon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-indigo-600 hover:text-indigo-700" />
              )}
            </button>
          )}
          ...
          <button
            ...
            className={`px-1.5 sm:px-2.5 py-1 sm:py-1.5 text-[10px] sm:text-xs font-bold rounded-lg sm:rounded-xl transition-all flex items-center gap-1 sm:gap-1.5 border whitespace-nowrap shrink-0 cursor-pointer ${
              isCurlActive
          ...
          >
            ...
            {isCurlActive && (
              <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-emerald-300 animate-pulse hidden sm:inline-block" />
            )}
          </button>
          ...
          {activePage === 'files' && (
            <>
              ...
              <button
                ...
                className="px-1.5 sm:px-2.5 py-1 sm:py-1.5 text-[10px] sm:text-xs font-medium bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-lg sm:rounded-xl transition-colors flex items-center gap-1 sm:gap-1.5 border border-slate-200/80 dark:border-slate-700/80 whitespace-nowrap shrink-0"
              >
          ...
          {currentUser ? (
            <div className="flex items-center gap-1 pl-1 border-l border-slate-200 dark:border-slate-800 shrink-0">
              <div
                onClick={() => {
                  if (isAdmin) onNavigate('users');
                }}
                className={`flex items-center gap-1 p-0.5 rounded-lg sm:rounded-xl transition-colors ${
              ...
              >
                <img
                  ...
                  onError={(e) => {
                    e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(currentUser.fullName || currentUser.username)}&background=3b82f6&color=fff`;
                  }}
                  className="w-5 h-5 sm:w-7 sm:h-7 rounded-md sm:rounded-lg object-cover ring-2 ring-blue-500/30 shrink-0"
                />
              ...
              <button
                onClick={onLogout}
                title="Sign out"
                className="p-1 sm:p-1.5 text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors shrink-0"
              >
            ...
            <button
              ...
              className="px-2 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-xs font-bold bg-amber-500 hover:bg-amber-600 text-slate-950 rounded-lg sm:rounded-xl shadow-sm transition-all flex items-center gap-1 whitespace-nowrap shrink-0"
            >
